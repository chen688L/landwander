<template>
  <div class="home">
    <div class="flex top">
      <div class="flex box">
        <img src="../../assets/static/users.png" />
        <div>
          <h2>用户总数</h2>
          <h3>{{ usernum }}</h3>
        </div>
      ...
      <div class="flex box">
        <img style="height: 60px" src="../../assets/static/land.png" />
        <div>
          <h2>土地总数</h2>
          <h3>{{ landnum }}</h3>
        </div>
    ...
    <div class="flex pic">
      ...
      <div>
        <div id="piechar"></div>
        <div>
          <div style="text-align: center; font-weight: bold">用户搜索记录</div>
...
export default {
  ...
  methods: {
    ...
    getNum() {
      ...
      this.$apis.checkChatNum({}).then((res) => {
    ...
    changePie() {
      // 获取组件实例
      const piechart = this.$echarts.init(document.getElementById("piechar"));
      var option = {
        title: {
          text: "土地类型数量分布图",
          left: "center",
    ...
    getSevenUser() {
      this.$apis
        .sevenDaysUser()
        .then((res) => {
          this.userDaysData = res.data.reverse();
          this.changetype();
        })
        .catch((err) => {
          console.log(err);
    ...
    getTypeNum() {
      this.$apis
        .LandTypeDays()
        .then((res) => {
          this.landTypeNum = res.data;
          this.landTypeNum.pop()
